...
gulp.task('build-iconfont', done => {
    gulp.src('src/iconfont/**/*')
        ...
        .pipe(gulp.dest('dist/js/iconfont', {
            mode: '0777'
        }))
        .on('end', () => {
            done && done();
...
gulp.task('weex-ecarx-ios', (done) => {
    weexErosPlatform = 'IOS'
    done()
});

gulp.task('weex-ecarx-android', (done) => {
    weexErosPlatform = 'ANDROID'
    done()
});

gulp.task('weex-ecarx-all', (done) => {
...
gulp.task('weex:js', ['build-js'], (done) => {
    var tasks = [];
    getFiles.getAllFiles(path.resolve(process.cwd(), 'dist/'), 'js').map((element) => {
    ...
    return merge(tasks).pipe(logger.time('build-weex-js')).on('end', (done) => {
        // 修复只能打pages的bug
        var _source = path.resolve(process.cwd(), 'dist/*'),
            _tmppath = path.resolve(process.cwd(), 'dist/_pages'),
            pagesPath=process.cwd() + '/dist/pages/',
            indexPath=process.cwd() + '/dist/index.js',
        ...
        gulp.src('src/iconfont/**/*')
            ...
            .on('end', function() {
                gulp.src('src/assets/**/*')
                    ...
                    .on('end', function() {
                        gulp.src('src/config/**/*')
                            .pipe(weexUtil.getConfigMd5())
                            .pipe(logger.time('build-config'))
                            .pipe(gulp.dest('dist/js/_pages/config', {
                                mode: '0777'
                            }))
                            .on('end', function() {
...
gulp.task('socket', (done) => {
    ...
    logger.log('enjoy it and ecarx loves you !');
    done()
});
...
gulp.task('handle-config-native', (done) => {
    ...
    weexErosPackUtil._writeJsonToIos('ecarx.native.json', nativeConfig)
    weexErosPackUtil._writeJsonToAndroid('ecarx.native.json', nativeConfig)
    if (isFirstReload) {
        isFirstReload = false
        logger.log('Reload ecarx.native.js done.')
    } else {
        logger.success('Reload ecarx.native.js done, you can rebuild your app to take effect.')
    }
    done()
})
...
gulp.task('watch', (done) => {
    ...
    gulp.watch('config/ecarx.native.js', ['handle-config-native'])
...
gulp.task('dev', (callback) => {
    gulpSequence('clean', ['build-assets', 'build-iconfont','build-config', 'build-mock', 'build-js'], ['start-mock', 'handle-config-native', "compatible-weex-debug", 'watch', 'socket'])(callback)
});
...
gulp.task('weex-ecarx:ios', (callback) => {
    gulpSequence('clean', 'weex-ecarx-ios', 'handle-config-native', 'weex:js')(callback)
});
gulp.task('weex-ecarx:android', (callback) => {
    gulpSequence('clean', 'weex-ecarx-android', 'handle-config-native', 'weex:js')(callback)
});
gulp.task('weex-ecarx:all', (callback) => {
    gulpSequence('clean', 'weex-ecarx-all', 'handle-config-native', 'weex:js')(callback)
});
