...
const TMP_NAME = 'ecarx-template-tmp';

var config = {
    name: 'update',
    explain: 'update ecarx-template file by path.',
    command: 'ecarx update _path',
    options: [{
        keys: ['-h', '--help'],
...
var questions = [{
    ...
    choices: [{
            ...
            value: "ecarx-template"
        },
        new inquirer.Separator(),
        {
            name: "simple | without default's widgets",
            value: "ecarx-template-simple",
...
var questions2 = [{
    type: 'list',
    name: 'result',
    message: 'What kind of ecarx template you developed now?',
    choices: [{
            name: "false",
            value: false
        },
...
function update(template, targetPath) {
    ...
    spinner.start();
    download(`welcome112s/${template}`, tmp, function(err) {
        spinner.stop()
...
function run() {
    ...
    if(argv._[1] === 'template' && argv._[2]) {
        update("ecarx-template", argv._[2])
        return
    } 
