...
var config = {
    name: 'init',
    explain: 'generate ecarx template.',
    command: 'ecarx init',
    options: [{
        keys: ['-h', '--help'],
        describe: 'read help.'
...
function create() {
    inquirer.prompt(questions).then(function(answers) {
        ...
        download(`welcome112s/${TEMPLATE}#${type}`, tmp, function(err) {
            spinner.stop()

            if (err) logger.fatal('Failed to download repo ' + TEMPLATE + ': ' + err.message.trim())
            changeFile(tmp + '/package.json', `${TEMPLATE}-name`, name)

            changeFile(tmp + '/config/ecarx.native.js', `${TEMPLATE}-name`, name)
            changeFile(tmp + '/config/ecarx.native.js', `${TEMPLATE}-version`, version)
            changeFile(tmp + '/platforms/android/gradle.properties', 'com.ecarx.wx', applicationID)
