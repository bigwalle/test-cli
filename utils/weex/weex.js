...
function getConfigMd5() {
    return through.obj(function(file, enc, cb) {
        ...
        var filePath = file.history[0],
            indexTag = filePath.indexOf(configTag),
        ...
        if (requestUrl) {
            __request.post(requestUrl, {
                ...
                if (!error && response.statusCode == 200) {
                    ...
                    logger.fatal('ecarx publish fail: %s', error);
                    reject('ecarx publish fail: %s', error)
                }
            });
        } else {
            jsonfile.writeFile(file, versionInfo, function(err) {
                if (err) {
                    logger.fatal('generate ecarx json error: %s', err);
                    reject('generate ecarx json error: %s', err)
                    return
                }
                resolve({ jsVersion, platform });
            });
...
function weexErosHandler({ jsVersion, platform }) {
    return new Promise((resolve) => {
        var params = {
            ...
            erosNative: require(path.resolve(process.cwd(), './config/ecarx.native.js')),
...
function _encrypt(data) {
    let _crypt = require('cryptlib'),
        ...
        iv = 'RjatRGC4W72PJXTE',
        key = _crypt.getHashSha256('ecarx loves you', 32)
